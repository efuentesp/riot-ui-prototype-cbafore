<inputbox>
	...
	<div class="form-group row">
		<div class="col-md-3 text-right">
			<label
				class="afobnmx_contlabel3 font-weight-bold"
				if={ opts.required == 'true'}
				for={opts.type}>{ opts.label }<font color="black">:*</font>
			</label>
			<label
				class="afobnmx_contlabel3 font-weight-bold"
				if={ opts.required == 'false' || ( ! opts.required && opts.label ) }
				for={opts.type}>{ opts.label }
			</label>
		</div>
		<div class="col-md-8">
			<input
				...
				value={opts.value}
			/>
			<div id={opts.id}2 class="invalid-feedback"></div>
		</div>
	</div>
