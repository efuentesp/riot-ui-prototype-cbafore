...
for (var j = 0; j < json.files.length; j++) {
  ...
  for (var c = 0; c < filenames[j].ids.length; c++) {
    localStorage.setItem(
      "rows_" + filenames[j].ids[c].id,
      JSON.stringify(filenames[j].ids[c].rows)
    );
    localStorage.setItem(
      "header_" + filenames[j].ids[c].id,
      JSON.stringify(filenames[j].ids[c].headers)
    );
    if (filenames[j].ids[c].actions !== "undefined") {
      localStorage.setItem(
        "actions_" + filenames[j].ids[c].id,
        JSON.stringify(filenames[j].ids[c].actions)
      );
    }
  }
}
...
var routes = [
  ...
  {
    route: "/banorte-cargos-ligados-contratos/",
    tag: "cargos-ligados-contratos"
  },
  ...
  {
    route: "/reservaciones-01-buscar-vuelo-seleccionado/",
    tag: "reservaciones-01-buscar-vuelo-seleccionado"
  },
